...
const UserInfoPage = () => {
   ...
   const { register, handleSubmit, formState: { errors },setValue } = useForm<IUserInFo>();
   ...
   const onSubmit =async (data:IUserInFo) => {
    ...
    try{
        ...
        await updateUser(object).unwrap().then(res=>{
            res
                 ...
                 <form action="" onSubmit={handleSubmit(onSubmit)}>
                    <div className='userinfo-header shadow-lg mb-[20px] flex justify-start gap-x-[10px] rounded-[10px] items-center p-[20px] bg-white w-full'>
                     ...
                     <div className='userinforTitle'>
                        <span className='main-title text-black font-bold text-[18px]'>Cập nhật ảnh đại diện mới</span>
                     </div>
                  </div>
                  <div className='userinfo-content gap-y-[10px] shadow-lg flex flex-wrap p-[20px] rounded-[10px] bg-white w-full'>
                     <div className='input-wrap flex  max-sm:flex-wrap max-sm:gap-y-[10px] justify-between w-full'>
                        <div className='w-[48%] max-sm:w-full'>
                           ...
                           {errors?.userName &&  (
                              <p className='error-message text-[13px] text-red-500'>{errors?.userName?.message?.toString()}</p>
                           )}
                        </div>
                        <div className='w-[48%] max-sm:w-full '>
                           <label>
                     ...
                     <button className='bg-[#455CC6] max-sm:w-full text-white text-center rounded-[10px] p-[10px]'>Cập nhật</button>
                  </div>
